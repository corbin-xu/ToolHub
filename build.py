...
2. 使用 Inno Setup 打包为安装程序 dist/ToolHub-1.7.exe（需已安装 Inno Setup）
"""
import subprocess
import sys
import os
...
DIST = ROOT / "dist"
EXE_NAME = "ToolHub.exe"
SPEC_FILE = ROOT / "ToolHub.spec"
...
def build_installer():
    ...
    if not iscc:
        ...
        print("[build] 跳过安装包步骤，仅保留 dist/ToolHub.exe")
        return False
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description="构建 ToolHub 可执行程序与安装包")
    parser.add_argument("--exe-only", action="store_true", help="仅打包 exe，不生成安装程序")
    parser.add_argument("--installer-only", action="store_true", help="仅生成安装程序（假定 dist/ToolHub.exe 已存在）")
