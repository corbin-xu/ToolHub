...
class UpdateCheckThread(QThread):
    ...
    def run(self):
        ...
        try:
            ...
            import json
            import ssl
            github_api_url = "https://api.github.com/repos/xgb819/ToolHub/releases/latest"
            ssl_context = ssl.create_default_context()
            ssl_context.check_hostname = False
            ssl_context.verify_mode = ssl.CERT_NONE
            request = urllib.request.Request(github_api_url)
            request.add_header('User-Agent', 'ToolHub/1.5')
            with urllib.request.urlopen(request, context=ssl_context, timeout=10) as response:
                data = json.loads(response.read().decode())
                latest_version = data.get('tag_name', '').lstrip('v')
                release_url = data.get('html_url', '') or ('https://github.com/xgb819/ToolHub/releases')
...
class DownloadUpdateThread(QThread):
    ...
    def run(self):
        try:
            ...
            req.add_header('User-Agent', 'ToolHub/1.5')
...
class KeywordAnalyzerGUI(QMainWindow):
    ...
    def generate_carton_marks_from_workbook(self, export_folder, label_type, export_date, export_year):
        ...
        try:
            ...
            for row in range(1, ws.max_row + 1):
                ...
                if not CartonMarkGenerator.is_valid_po_num(po_number):
                    continue
            ...
            for row in range(1, ws.max_row + 1):
                ...
                po_number = values[row + 2]
                if not CartonMarkGenerator.is_valid_po_num(po_number):
                    print(f"[DEBUG] 行{row}: 采购单号'{po_number}'不是10位或12位数字，跳过")
    ...
    def _generate_shantou_b_carton_marks(self, ws, export_folder):
        ...
        try:
            ...
            for row in range(1, ws.max_row + 1):
                ...
                #  C列: row+1 行 -> 供应商
                #       row+2 行 -> 入库库房
                #       row+3 行 -> EPL采购单号（EPL+数字，位数不超过模板上限）
                #       row+4 行 -> 需求单号（PR+数字，位数不超过模板上限）
                #       row+5 行 -> 产品规格
    ...
    def generate_box(self):
        ...
        # 验证采购单号
        if not CartonMarkGenerator.is_valid_po_num(po_num):
            QMessageBox.warning(self, "警告", "采购单号必须是10位或12位数字")
            return
...
class BoxDialog(QDialog):
    ...
    def init_ui(self):
        ...
        self.box_po_input.setPlaceholderText("10位或12位数字")
