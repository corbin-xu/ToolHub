...
class UpdateCheckThread(QThread):
    ...
    def run(self):
        ...
        try:
            ...
            request.add_header('User-Agent', 'ToolHub/1.7')
...
class DownloadUpdateThread(QThread):
    ...
    def run(self):
        try:
            ...
            req.add_header('User-Agent', 'ToolHub/1.7')
...
class KeywordAnalyzerGUI(QMainWindow):
    ...
    def generate_carton_marks_from_workbook(self, export_folder, label_type, export_date, export_year):
        ...
        try:
            ...
            for row in range(1, ws.max_row + 1):
                ...
                po_number = values[row + 2]
                if not CartonMarkGenerator.is_valid_po_num(po_number):
                    print(f"[DEBUG] 行{row}: 采购单号'{po_number}'不是7到20位数字，跳过")
    ...
    def generate_box(self):
        ...
        if not CartonMarkGenerator.is_valid_po_num(po_num):
            QMessageBox.warning(self, "警告", "采购单号必须是7到20位数字")
            return
