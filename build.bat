...
if not exist "dist\jd-supplier-label-generator.exe" (
    echo [build] 未找到 dist\jd-supplier-label-generator.exe
    exit /b 1
)
echo [build] 已生成 dist\jd-supplier-label-generator.exe
...
if "%ISCC%"=="" (
    ...
    echo [build] 仅完成 exe 打包: dist\jd-supplier-label-generator.exe
    exit /b 0
)
...
echo [build] 完成。安装包: dist\jd-supplier-label-generator-1.4.exe
exit /b 0
